...
require_once __DIR__ . './functions/custom-handlebars.php';

function crimson_parse_handlebars( array $matches = array() ) {
...
function crimson_handlebars($field) {
	$value = preg_replace_callback( '#\{{(.*)\}}#Ui', 'crimson_parse_handlebars', $field);
	if ( file_exists(  './functions/custom-handlebars.php' ) ) {
		$value = preg_replace_callback( '#\{{(.*)\}}#Ui', 'crimson_custom_handlebars', $value);
	}
	return $value;
}
?>
